...
//处于安全考虑必须为secretCode赋特定的值,不能使用默认值,因此不可以使用之前方案:简单覆盖NSObject的init方法,并使用某个默认值作为实参,调用initWithCode:方法
//如何解决?即使不覆盖init方法,BNRWallSafe实例一样可以相应init消息,然后不知情的程序员使用下面的方法创建BNRWallSafe实例
//BNRWallSafe *ws = [[BNRWallSafe alloc] init];
//最佳解决方案是覆盖父类的指定初始化方法,然后告知程序员不能调用
//-(instancetype)init{
//    [NSException raise:@"xxx" format:@"xxxx"];
//}
